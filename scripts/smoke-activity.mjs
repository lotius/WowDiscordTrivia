...
try {
  ...
  const host = await emit(hostSocket, "room:activity", {
    instanceId, name: "Host", discordUserId: "user-host"
  });
  const guest = await emit(guestSocket, "room:activity", {
    instanceId, name: "Guest", discordUserId: "user-guest"
  });
  ...
  console.log(`✓ one voice channel -> one room (${host.state.code}), no code exchanged`);
  ...
  const scoreBefore = afterReveal.players.find((p) => p.discordUserId === "user-guest").score;
  assert(scoreBefore > 0, "Guest scored nothing, so the reconnect check would prove nothing.");
  console.log(`✓ guest banked ${scoreBefore} points`);
  ...
  const guestAfter = returned.state.players.find((p) => p.discordUserId === "user-guest");
  ...
  assert(
    returned.state.players.filter((p) => p.discordUserId === "user-guest").length === 1,
    "Reconnect created a duplicate player."
  );
  console.log(`✓ reconnect mid-game kept ${guestAfter.score} points and did not duplicate the player`);
  ...
  const late = await emit(lateSocket, "room:activity", {
    instanceId, name: "Latecomer", discordUserId: "user-late"
  });
  assert(late.state.code === host.state.code, "Latecomer landed in a different room.");
  assert(
    late.state.players.some((p) => p.discordUserId === "user-late"),
    "Latecomer was not added to the in-progress game."
  );
  console.log("✓ latecomer joined a game already in progress");
